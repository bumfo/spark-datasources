...
import org.apache.spark.sql.fourmc.{FourMcPlanner, FourMcScan, FourMcScanBuilder, FourMcSchemaAwareDataSource, FourMcSliceReader, FourMcTable}
...
class FourMcCSVTable(
  ...
  override protected def buildScanBuilder(): FourMcScanBuilder =
    new FourMcCSVScanBuilder(sparkSession, fileIndex, options, schema, planner)
  ...
  override def inferSchema(files: Seq[org.apache.hadoop.fs.FileStatus]): Option[StructType] = {
    ...
    val ds = planner.datasetOfLines(Some(parsed.charset))
...
final case class FourMcCSVScan(
